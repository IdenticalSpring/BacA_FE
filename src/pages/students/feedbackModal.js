...
import feedbackService from "services/feedbackService"; // Sử dụng feedbackService đã cung cấp
import { colors } from "pages/teachers/teacherPage";

const { Title } = Typography;
const { TextArea } = Input;

const StudentFeedbackModal = ({ visible, onClose, studentId }) => {
  ...
  const handleSubmit = async (values) => {
    try {
      setLoading(true);
      const feedbackData = {
        studentId: studentId, // Gửi studentId trong dữ liệu
        title: values.title,
        description: values.description,
      };
      await feedbackService.createFeedback(feedbackData); // Sử dụng feedbackService
  ...
  return (
    <Modal
      title={<Title level={4}>Create Student Feedback</Title>}
      open={visible}
      onCancel={onClose}
      footer={null}
      width="50%"
...
StudentFeedbackModal.propTypes = {
